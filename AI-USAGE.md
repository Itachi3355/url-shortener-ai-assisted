...
| Horizontal scrolling on the link table | Reported by a user; my earlier "fix" was wrong | I had contained the overflow by making the table scroll inside its own card, which stops the *page* scrolling sideways but still leaves the reader swiping to see one link. **Containing an overflow is not fixing it.** Rows now reflow into stacked cards below 820px, and `table-layout:fixed` means the table can never demand more width than its container at any size |
